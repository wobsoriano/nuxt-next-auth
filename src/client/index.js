...
export function useSession(session) {
  ...
  if (store && 'session' in store.state.auth) {
    return _useSessionHook(store.state.auth.session)
  }
 ...
 */
 function BroadcastChannel(name = "nextauth.message") {
    return {
       ...
       * @param {(message: import("next-auth/types/internals/client").BroadcastMessage) => void} onReceive
       */
      receive(onReceive) {
        if (typeof window === "undefined") return
        window.addEventListener("storage", (event) => {
          if (event.key !== name) return
          /** @type {import("next-auth/types/internals/client").BroadcastMessage} */
